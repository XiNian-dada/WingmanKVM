...
选择 `Virtual Monitor = 1280×720 @ 60Hz` 会给被控机呈现只声明该模式的 EDID，并通过 HPD 低/高模拟重新连接。选择 `Capture = Native` 时，只有 EDID RAM 已写入并回读成功，V4L2 才跟随为 `1280×720 @ 60fps`。固定 Capture 模式仍可用于“1080p 桌面、720p 网络视频”。Viewer 设置只改变 CSS 显示，不参与 EDID 或采集配置。
...
- WingmanKVM 能确认 EDID RAM 回读和实际 V4L2 输出格式，但 MS2130 没有标准接口报告 HDMI Source 的当前输入时序。因此状态页不会把 UVC 720p 表述成“GPU 输出已被测得为 720p”。
...
8. 拉高 HPD，等待 Source 重新建立信号；
9. 恢复 V4L2 采集。

步骤 3 之后的任何错误都会尽力写回备份并恢复原始控制寄存器。硬件事务成功后才原子保存配置；若保存失败，会执行第二次 HPD 周期恢复切换前的 RAM。切换操作由全局配置锁串行化，不会与另一项网页配置并发写设备。
...
3. 确认页面显示“EDID RAM 已回读校验，HPD 已恢复”；
