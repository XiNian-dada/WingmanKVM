...
for ATTR in file ro cdrom removable forced_eject; do
  if [ -e "$LUN/$ATTR" ]; then
    ...
    sudo chmod g+rw "$LUN/$ATTR"
  fi
done
```

`forced_eject` 并非所有内核都提供，因此只能作为可选能力。不同内核/configfs 挂载方式可能会重置权限，应在 Gadget 初始化服务中执行，而不是依赖一次性的手工修改。

ISO 会以 `cdrom=1, ro=1` 暴露；IMG 以 `cdrom=0` 暴露，并可选择 `ro=0` 读写。读写镜像文件本身必须允许 `wingmankvm` 用户写入：

```bash
sudo chown wingmankvm:wingmankvm /var/lib/wingmankvm/images/example.img
sudo chmod 0600 /var/lib/wingmankvm/images/example.img
```

如果镜像目录不在 `/var/lib/wingmankvm`，还需要给 systemd unit 增加对应的 `ReadWritePaths=`；否则 `ProtectSystem=strict` 会阻止上传和读写。正在通过 Gadget 提供的可写 IMG 不得同时在控制机上 loop mount。弹出前先在被控机中卸载文件系统，强制弹出可能造成文件系统损坏。
